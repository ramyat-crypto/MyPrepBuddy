...
<body>
 ...
 <div style='transform: scale(0.90); position: relative; top: 2px; background-color: rgba(177,156,217,0.4)'>
            ...
            $result = $con->query("Select * from technical_table LIMIT $start_from, $per_page_record") or die($con->error);
                  ...
                  ?>
                    <div style="padding-left: 30px">
                      ...
                      <form>
                        ...
                        <button class="submitBtn" type="button" onclick="validate(choice.value,'<?php echo $row["Answer"]; ?>','<?php echo $row["T_id"] ?>')">Submit</button>
                      </form>
                      <div id="<?php echo $row['T_id']?>"></div>
                      <hr/>
                  </div>
          ...
          <?php  
            $query = "SELECT COUNT(*) FROM technical_table";     
        ...
        echo "</br>";     
            ...
            if($page>=2){   
                echo "<a href='technical_query.php?page=".($page-1)."'>  Prev </a>";   
            }       
                       
            for ($i=1; $i<=$total_pages; $i++) {   
              if ($i == $page) {   
                  $pagLink .= "<a class=active href='technical_query.php?page="  
                                                    .$i."'>".$i." </a>";   
              }               
              else  {   
                  $pagLink .= "<a href='technical_query.php?page=".$i."'>   
                                                    ".$i." </a>";     
              }   
            };     
            echo $pagLink;   
            ...
            if($page<$total_pages){   
                echo "<a href='technical_query.php?page=".($page+1)."'>  Next </a>";   
            }   
